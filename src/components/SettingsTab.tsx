...
import { exit } from '@tauri-apps/plugin-process'; // Nếu muốn dùng lệnh thoát app
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pr-1 custom-scrollbar">
      ...
      <div className="space-y-6 max-w-4xl px-2 pb-10">
        ...
        <div className={`rounded-2xl p-6 transition-all duration-300 ${glassPanel(isDark)}`}>
            {/* ... (Nội dung toggle thông báo) ... */}
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className={`text-lg font-bold ${textPrimary(isDark)}`}>Thông báo hệ thống</h3>
                <p className={`text-sm mt-1 max-w-md leading-relaxed ${textSecondary(isDark)}`}>
                  Hiển thị thông báo (Notification) trên màn hình máy tính khi một công cụ chạy xong hoặc gặp lỗi.
                </p>
              </div>
            </div>
            <button 
              onClick={toggleNotifications}
              className={`relative flex items-center w-14 h-8 rounded-full p-1 cursor-pointer transition-colors duration-300 border ${
                notificationsEnabled ? (isDark ? 'bg-cyan-500/20 border-cyan-500/50' : 'bg-sky-500/20 border-sky-500/50') : (isDark ? 'bg-white/10 border-white/10' : 'bg-black/10 border-black/10')
              }`}
            >
              <motion.div layout className={`w-6 h-6 rounded-full shadow-md ${notificationsEnabled ? (isDark ? 'bg-cyan-400' : 'bg-sky-500') : (isDark ? 'bg-white/40' : 'bg-white')}`} animate={{ x: notificationsEnabled ? 24 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
            </button>
          </div>
        </div>
